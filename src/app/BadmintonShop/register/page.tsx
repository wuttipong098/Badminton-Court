...
const RegisterPage = () => {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      Promise.all(fileReaders)
        .then((base64Strings) => setImages((prev) => [...prev, ...base64Strings]))
        .catch((error) => {
          MySwal.fire({
            icon: "error",
            title: "ข้อผิดพลาด",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!role) {
      MySwal.fire({
        ...
        text: "กรุณาเลือกบทบาท (ผู้เล่นหรือเจ้าของสนาม)",
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-gray-300">
      <div className="flex w-[700px] h-[750px] bg-white rounded-xl shadow-lg overflow-hidden">
          ...
          <form className="w-full px-8" onSubmit={handleSubmit}>
            <div className="flex justify-center gap-2 mb-4">
              <div className="flex flex-col items-center">
                <div
                  className={`flex flex-col items-center bg-white rounded-lg p-2 cursor-pointer ${
                    role === "user" ? "border-4 border-yellow-400" : ""
                  }`}
              ...
              <div className="flex flex-col items-center">
                <div
                  className={`flex flex-col items-center bg-white rounded-lg p-2 cursor-pointer ${
                    role === "owner" ? "border-4 border-yellow-400" : ""
                  }`}
            ...
            <div className="mb-2 relative">
              <input
                ...
                className={`w-full p-1.5 pr-8 bg-gray-200 text-gray-800 border ${
                  passwordError ? "border-red-500" : "border-gray-300"
                } rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 text-sm`}
            ...
            <div className="mb-2">
              <label className="text-white text-sm mb-1 block">อัปโหลดรูปภาพสนาม</label>
              <input
                type="file"
                accept="image/*"
                multiple
